/** Adds a new entry. */
export interface ModDataAddEntryParams {
    /** data instance id */
    databaseid: number | null;
    /** Group id, 0 means that the function will determine the user group */
    groupid?: number | null;
    /** The fields data to be created */
    data: Array<{
        /** The field id. */
        fieldid: number | null;
        /** The subfield name (if required). */
        subfield?: string | null;
        /** The contents for the field always JSON encoded. */
        value: string | null;
    }>;
}

export interface ModDataAddEntryReturns {
    /** True new created entry id. 0 if the entry was not created. */
    newentryid: number | null;
    generalnotifications: string | null[];
    fieldnotifications: Array<{
        /** The field name. */
        fieldname: string | null;
        /** The notification for the field. */
        notification: string | null;
    }>;
    /** list of warnings */
    warnings?: Array<{
        /** item */
        item?: string | null;
        /** item id */
        itemid?: number | null;
        /** the warning code can be used by the client app to implement specific behaviour */
        warningcode: string | null;
        /** untranslated english message to explain the warning */
        message: string | null;
    }>;
}

export type ModDataAddEntryReturn = ModDataAddEntryReturns;
export type mod_data_add_entry_returns = ModDataAddEntryReturns;
