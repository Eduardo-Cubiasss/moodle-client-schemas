/** List of enrolled courses for the given timeline classification (past, inprogress, or future). */
export interface CoreCourseGetEnrolledCoursesByTimelineClassificationParams {
    /** future, inprogress, or past */
    classification: string | null;
    /** Result set limit */
    limit?: number | null;
    /** Result set offset */
    offset?: number | null;
    /** Sort string */
    sort?: string | null;
    /** Used when classification = customfield */
    customfieldname?: string | null;
    /** Used when classification = customfield */
    customfieldvalue?: string | null;
    /** The value a user wishes to search against */
    searchvalue?: string | null;
    /** Array of the only field names that need to be returned. If empty, all fields will be returned. */
    requiredfields?: string | null[];
}

export interface CoreCourseGetEnrolledCoursesByTimelineClassificationReturns {
    /** Course */
    courses: Array<{
        /** id */
        id: number;
        /** fullname */
        fullname: string;
        /** shortname */
        shortname: string;
        /** idnumber */
        idnumber: string;
        /** summary */
        summary: string | null;
        /** summary format (1 = HTML, 0 = MOODLE, 2 = PLAIN, or 4 = MARKDOWN) */
        summaryformat?: number | null;
        /** startdate */
        startdate: number;
        /** enddate */
        enddate: number;
        /** visible */
        visible: boolean;
        /** showactivitydates */
        showactivitydates: boolean | null;
        /** showcompletionconditions */
        showcompletionconditions: boolean | null;
        /** pdfexportfont */
        pdfexportfont: string | null;
        /** fullnamedisplay */
        fullnamedisplay: string;
        /** viewurl */
        viewurl: string;
        /** courseimage */
        courseimage: string;
        /** progress */
        progress?: number;
        /** hasprogress */
        hasprogress: boolean;
        /** isfavourite */
        isfavourite: boolean;
        /** hidden */
        hidden: boolean;
        /** timeaccess */
        timeaccess?: number;
        /** showshortname */
        showshortname: boolean;
        /** coursecategory */
        coursecategory: string;
    }>;
    /** Offset for the next request */
    nextoffset: number | null;
}

export type CoreCourseGetEnrolledCoursesByTimelineClassificationReturn = CoreCourseGetEnrolledCoursesByTimelineClassificationReturns;
export type core_course_get_enrolled_courses_by_timeline_classification_returns = CoreCourseGetEnrolledCoursesByTimelineClassificationReturns;
